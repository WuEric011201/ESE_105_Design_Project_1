...
% distances = abs(zscore(distances));
%  iqRange = iqr(distances);
%  sevenfive = prctile(distances, 75);
%  twentyfive = prctile(distances, 25);
%outliers(distances>2) =1;

% Find the silhouette value of each data point
s = silhouette(testOG,predictions,'Euclidean');
%   iqRange = iqr(s);
%   sevenfive = prctile(s, 75);
%   twentyfive = prctile(s, 25);

% find the outlier if the s value is smaller than a given number
  outliers(s<-0.2) =1;
...
for ind=1:len
    ...
    centr=testOG(outlierIndexes(ind),[1:784]);
